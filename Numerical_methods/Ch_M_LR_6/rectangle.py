...
# обчислює значення функції за формулою прямокутників
def rectangles(a, b, n, f):
    h = (b - a) / n # розраховуємо крок
    s = 0           # змінна для зберігання суми

    for i in range(n): # цикл по вузлам
        x_left = a + i * h    # ліва межа відрізка
        x_right = x_left + h  # права межа
        x_mid = (x_left + x_right) / 2 # обчислення середнього х

        s += f(x_mid) # обчислення у від середнього х

    return s * h # повертаємо результат обчислень
