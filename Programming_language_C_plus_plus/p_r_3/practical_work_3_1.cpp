...
int main()                                 // головна функція програми
{
	...
	std::string* arr = nullptr;            // оголошуємо вказівник який ні на що не вказує

	try                                    // спробуємо виконати наступний блок коду
	{
		arr = new std::string[N];          // динамічне виділення пам'яті
	}
	catch (const std::bad_alloc& e)        // якщо сталася помилка при виділенні пам'яті
	{
		std::cerr << "\nPomylka pru vydilenni pam'yati: " << e.what() << std::endl;
		return 1;
	}
