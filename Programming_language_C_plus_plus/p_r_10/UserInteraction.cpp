...
// функція для виведення історії ігор
void showHistory()
{
    std::ifstream gameHistory("GameHistory.txt");       // відкриваємо файл у режимі читання

    if (!gameHistory)                                   // якщо файл не відкрито
    {
        std::cout << "\nPomylka pry vidkrytti faylu GameHistory.txt!\n";
    }
    else
    {
        std::string line;                               // змінна для зберігання одного рядка з файлу

        while (std::getline(gameHistory, line))         // читаємо файл рядок за рядком до самого кінця
        {
            std::cout << line << "\n";                  // виводимо кожен прочитаний рядок на екран
        }

        gameHistory.close();                            // закриваємо файл після читання
    }
}